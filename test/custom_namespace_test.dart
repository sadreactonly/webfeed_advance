...
void main() {
  test('Loads tags into customNamespace map.xml', () {
    ...
    expect(feed.title,
        'Latest Kijiji ads. Location: Regina. Category: Cars & Trucks');
    expect(feed.items!.length, 4);

    var mapIndex = 0;
    for (var item in feed.items!) {
      expect(item.customNamespace!.valueForKey('g-core:price'),
          expectValues[mapIndex]['g-core:price']);
      expect(item.customNamespace!.valueForKey('geo:lat'),
          expectValues[mapIndex]['geo:lat']);
      expect(item.customNamespace!.valueForKey('geo:long'),
          expectValues[mapIndex]['geo:long']);
